...
import { RequestEmailTemplate } from "@/email/requestEmailTemplate";

const resend = new Resend(process.env.RESEND_API_KEY);

export async function POST(request: Request) {
  const { name, email, phone, topic, message, type } = await request.json();
  ...
  const { data, error } = await resend.emails.send({
    ...
    replyTo: email,
    subject: `[닷셰프 웹사이트] ${name}님의 ${isRequest ? "견적 요청" : "문의"}`,
    react: isRequest
      ? RequestEmailTemplate({ name, email, phone, topic, message })
